...
public static class INIHelper
{
    ...
    /// <PARAM name="Value">值（字符串）</PARAM>
    public static void WriteObject(string Section, object Value, string path) 
    {
        Type type = Value.GetType();
        foreach (var propertyInfo in type.GetFields())
        { 
            Write(Section, propertyInfo.Name, propertyInfo.GetValue(Value).ToString(), path);
        }
    }
    /// <summary>
    /// 读取INI数据
    ...
    /// <PARAM name="Path">值名</PARAM>
    /// <returns>值（字符串）</returns>
    public static T ReadObject<T>(string Section, string path)
    {
        string[] keys;
        string[] values;
        int ret  = GetAllKeyValues(Section,out keys,out values,path);
        Type type = typeof(T);
        T t = (T)Activator.CreateInstance(type );
            
        Dictionary<String, int> keyss = keys.Select((key,i)=>new Tuple<String,int>(key,i)).ToDictionary(key => key.Item1, key => key.Item2);
        foreach (var fieldInfo in type.GetFields())
        {
            String key = fieldInfo.Name;
            if (keyss.TryGetValue(key, out int index))
            {
                    
                Type pt = fieldInfo.FieldType;         //获取到属性的type
                string value = values[index];
                String dtlType = fieldInfo.FieldType.Name;
                fieldInfo.SetValue(t, fieldInfo.FieldType.Name.Contains("Nullable")
                                ? Convert.ChangeType(value, Nullable.GetUnderlyingType(pt))
